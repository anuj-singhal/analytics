...
def test_count_available_vaccines():
    ...
    assert job.count_available_vaccines(create_sample(sample4)) == 3
...
def test_total_vaccinations_per_country():
    total_vaccinations = job.total_vaccinations_per_country(create_sample(sample1)).rdd.collectAsMap()
